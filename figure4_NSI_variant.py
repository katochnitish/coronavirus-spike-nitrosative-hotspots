"""
Figure 4: Variant-specific Nitrosative Susceptibility Index analysis.
...
Purpose
-------
This script calculates position-wise Nitrosative Susceptibility Index profiles
for the SARS-CoV-2 spike protein and compares Wuhan, Alpha, Beta, Delta, and
Omicron BA.2. It also quantifies variant-specific changes relative to Wuhan,
summarizes NSI by spike domain, identifies residue-level hotspots, and creates
individual and combined publication figures.

Input
-----
02_CuratedRawData/combined_curated_aligned.fasta

The input must be a multiple-sequence FASTA alignment containing recognizable
headers for Wuhan, Alpha, Beta, Delta, and Omicron BA.2 sequences. Alignment
columns corresponding to gaps in the Wuhan reference are removed before
position-wise analysis. The spike sequence is evaluated over 1,273 residues.

Main calculations
-----------------
1. Reads and classifies aligned FASTA records by viral group.
2. identifies the Wuhan-Hu-1 reference sequence and removes its gap columns.
3. Assigns every residue position to a defined spike domain.
4. Determines the consensus amino acid for each variant and position.
5. Calculates within-group Shannon entropy and applies a seven-residue rolling
   mean.
6. Calculates mutation frequency relative to Wuhan.
7. Assigns residue reactivity, approximate exposure, domain, and ACE2-interface
   scores.
8. Calculates raw NSI as:
...
9. Min-max normalizes raw NSI to the range 0 to 1.
10. Smooths normalized NSI using a seven-residue rolling window.
11. Calculates delta NSI at each position as:

       Delta NSI = variant smoothed NSI - Wuhan smoothed NSI

12. Summarizes NSI and delta NSI by spike domain and selects residue hotspots
    using NSI, delta NSI, reactivity, interface status, and predefined key sites.

Plots generated
---------------
Figure 4A, line plot:
    Smoothed NSI profiles across spike amino acid positions for Wuhan and the
    four variants, with spike-domain annotations.

Figure 4B, heatmap:
    Position-wise delta NSI relative to Wuhan for Alpha, Beta, Delta, and
    Omicron BA.2.

Figure 4C, violin plots:
    Domain-wise distributions of smoothed NSI for Wuhan and each variant.

Combined Figure 4:
    A multi-panel figure containing the NSI curves, delta NSI heatmap, and
    domain-wise violin plots.
...
03_Results/Figure4_positionwise_variant_NSI.csv
    Position-wise NSI components, raw NSI, normalized NSI, and smoothed NSI.

03_Results/Figure4_delta_NSI_vs_Wuhan.csv
    Position-wise smoothed NSI and delta NSI relative to Wuhan.

03_Results/Figure4_domainwise_variant_NSI_summary.csv
    Domain-level descriptive statistics for NSI and delta NSI.

03_Results/Figure4_variant_NSI_hotspot_amino_acid_table.csv
    Ranked residue-level NSI hotspots for each group.
...
Notes
-----
The NSI is a weighted computational index defined in this script. Approximate
exposure and domain contributions are rule-based scores rather than values from
an experimentally resolved structure or molecular dynamics simulation.
"""
...
def plot_variant_curves(rows, ax=None, save=True):
    ...
    for pos in label_positions:
        ...
        ax.axvline(pos, color="#777777", linestyle=":", lw=0.9, alpha=0.55, zorder=1)
    ...
    if own_fig and save:
        plt.tight_layout(rect=[0.035, 0.10, 0.995, 0.98])
...
def plot_delta_heatmap(delta_rows, ax=None, save=True):
    ...
    for pos in label_positions:
        label = KEY_POSITIONS[pos]
        ax.axvline(pos, color="#222222", linestyle=":", lw=0.8, alpha=0.70)
    ...
    if own_fig and save:
        plt.tight_layout(rect=[0.04, 0.15, 0.96, 0.92])
